  ...
  module Filters
  	...
  	def pager_links(pager)
		...
		if pager['previous_page'] || pager['next_page']
			...
			html = '<nav class="paginate">'

			if pager['next_page'] 
				html << "<a class=\"prev\" href=\"/#{pager['category']}/page#{pager['next_page']}\" title=\"Older posts\">Older entries</a>"
			end

			if pager['previous_page']
				
				if pager['previous_page'] == 1
					html << "<a class=\"next\" href=\"/#{pager['category']}/\" title=\"Newer posts\">Newer posts</a>"
				else
					html << "<a class=\"next\" href=\"/#{pager['category']}/page#{pager['previous_page']}\" title=\"Newer posts\">Newer posts</a>"
				end
